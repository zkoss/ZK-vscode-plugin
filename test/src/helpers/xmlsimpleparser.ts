...
export default class XmlSimpleParser {
    ...
    public static getSchemaXsdUris(xmlContent: string, documentUri: string): Promise<string[]> {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const sax = require("sax");
        const parser = sax.parser(true);
    ...
    public static getScopeForPosition(xmlContent: string, offset: number): Promise<XmlScope> {
        ...
        return new Promise<XmlScope>(
            (resolve) => {
                let result: XmlScope,
                    previousStartTagPosition = 0;
                const isAutoClose = xmlContent.endsWith('></\n'),
                    updatePosition = () => {

                        if ((parser.position >= offset) && !result || isAutoClose) {
                            ...
                            if ('>' == xmlContent.substring(xmlContent.length - 2, xmlContent.length - 1)) {
                                result.tagName = content.substring(1, normalizedContent.indexOf(">"));
                            } else if (isAutoClose) {
                                result.tagName = xmlContent.substring(xmlContent.indexOf("<") + 1, xmlContent.indexOf(">"));
                            }

                            if ((content.lastIndexOf(">") >= content.lastIndexOf("<")) && content.lastIndexOf("<") != 0) {
                                result.context = "text";
                            } else {
                                ...
                                } else if (normalizedContent.includes('apply') || normalizedContent.includes('init') || normalizedContent.includes('class')) {
                                    result.context = "link";
                                } else {
