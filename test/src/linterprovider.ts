...
import { languageId, globalSettings, xsdUriString } from './extension';
...
export default class XmlLinterProvider implements vscode.Disposable {
    ...
    private async triggerLint(textDocument: vscode.TextDocument): Promise<void> {
        ...
        try {
            ...
            const xsdFileUris = (await XmlSimpleParser.getSchemaXsdUris(documentContent, textDocument.uri.toString(true)))
            ...
            if (xsdFileUris.length === 0) {
                xsdFileUris.push({ uri: vscode.Uri.parse(xsdUriString), parentUri: vscode.Uri.parse(xsdUriString) });
            }

            const currentTagCollections: XmlTagCollection[] = [];

            const currentUriPair = { uri: vscode.Uri.parse(xsdUriString), parentUri: vscode.Uri.parse(xsdUriString) };
            ...
            try {
                ...
                if (this.schemaPropertiesArray.length <= 2) {
